...
impl GeometryRouter {
        ...
        for opening in openings.iter() {
            match opening {
                ...
                OpeningType::NonRectangular(opening_mesh) => {
                    ...
                    // FIX (2026-03-18): Guard CSG against tiny / non-intersecting openings.
                    //
                    ...
                    // misclassifies these as floor openings, forcing them into the CSG path.
                    // The csgrs BSP tree then destroys the (potentially 400+ triangle) wall
                    // mesh — reducing it to ~19 triangles — because tiny operands trigger
                    // numerical instability in the BSP split/merge.
                    //
                    // Three guards:
                    ...
                    // If the opening volume is negligible compared to the wall, skip it.
    ...
    pub fn generate_opening_reveals(
        ...
        // Get wall mesh in world space for wall thickness computation
        let wall_mesh = match self.process_element(element, decoder) {
            Ok(m) if !m.is_empty() => m,
            _ => return Vec::new(),
        };
